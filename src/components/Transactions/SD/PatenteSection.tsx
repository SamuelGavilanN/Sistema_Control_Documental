// src/components/Transactions/SD/PatenteSection.tsx

import React, { useState, useRef, useEffect } from "react";
import { patentes, Patente, crearPatente } from "../../../data/patentes";

interface Props {
  titulo: string;
  value: string;
  onChange: (value: string) => void;
  onPatenteIdChange?: (id: string) => void;
...
const PatenteSection: React.FC<Props> = ({
  titulo,
  value,
  onChange,
  onPatenteIdChange,
  ...
  useEffect(() => {
    if (patenteSeleccionada && onPatenteIdChange) {
      onPatenteIdChange(patenteSeleccionada.id);
    } else if (!value && onPatenteIdChange) {
      onPatenteIdChange("");
    }
  }, [patenteSeleccionada, value, onPatenteIdChange]);
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const filtered = patentes.filter((p) =>
      p.numero_patente.toUpperCase().includes(newValue.toUpperCase())
    );
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === "Enter") {
      if (isOpen && highlightIndex >= 0) {
        ...
        if (bestMatch) {
          e.preventDefault();
          onChange(bestMatch.numero_patente);
          setIsOpen(false);
        }
      }
    } else if (e.key === "Tab") {
      const bestMatch = getBestMatch(value);
      if (bestMatch && value !== bestMatch.numero_patente) {
        e.preventDefault();
        onChange(bestMatch.numero_patente);
        setIsOpen(false);
        return;
      ...
      if (!isOpen && value) {
        const filtered = patentes.filter((p) =>
          p.numero_patente.toUpperCase().includes(value.toUpperCase())
        );
        setSuggestions(filtered);
        setIsOpen(true);
        setHighlightIndex(0);
  ...
  const handleAddPatente = async () => {
    try {
      ...
      if (nuevaPatente.numero_patente) {
        const creada = await crearPatente({
          numero_patente: nuevaPatente.numero_patente,
        ...
        if (onPatenteIdChange) onPatenteIdChange(creada.id);
      }
      setShowAddForm(false);
      setNuevaPatente({});
      actualRef.current?.focus();
    } catch (error: any) {
      alert("Error al guardar patente: " + error.message);
    } finally {
      setGuardando(false);
    }
  };
      ...
      <div className="compact-content">
        <div className="autocomplete-wrapper" ref={wrapperRef}>
          <input
            ...
            placeholder="Buscar o escribir patente..."
          />
