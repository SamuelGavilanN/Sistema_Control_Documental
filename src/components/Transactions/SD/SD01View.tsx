...
const SD01View: React.FC = () => {
  ...
  const handleConductorChange = (value: string) => {
    ...
    } else { setShowConductorSuggestions(false); }
  };

  const selectConductor = (c: any) => { setConductor(c.nombre_completo || ''); setShowConductorSuggestions(false); };
  ...
  const handlePatentePChange = (value: string) => {
    ...
    } else { setShowPatentePSuggestions(false); }
  };

  const selectPatenteP = (p: any) => { setPatentePrincipal(p.numero_patente || ''); setShowPatentePSuggestions(false); };
  ...
  const handlePatenteAChange = (value: string) => {
    ...
    } else { setShowPatenteASuggestions(false); }
  };

  const selectPatenteA = (p: any) => { setPatenteAdicional(p.numero_patente || ''); setShowPatenteASuggestions(false); };
  ...
  const handleGuardar = async (estado: string) => {
    ...
    try {
      ...
      const respDoc = await fetch(API_URL + '/sd01_documentos', {
        ...
        body: JSON.stringify({
          id_documento: idDocumento, conductor_id: conductorData?.id || null,
          patente_principal_id: patentePData?.id || null, patente_adicional_id: patenteSData?.id || null,
          fecha_programacion: fechaProgramacion, sello_lateral: selloLateral, sello_adicional: selloAdicional,
          administrativo: (user?.nombre || '') + ' ' + (user?.apellido || ''),
          observaciones: observaciones, estado: estado, creado_por: user?.id,
        }),
      });
  ...
  const limpiarFormulario = () => {
    ...
    setSelloLateral(''); setSelloAdicional(''); setObservaciones('');
  ...
  return (
    <div className="sd01-view">
      ...
      <div style={{ display: 'flex', gap: '8px', marginBottom: '20px', flexWrap: 'wrap' }}>
        <button className="sd01-btn-nueva" onClick={() => { limpiarFormulario(); setShowCrearModal(true); }}>
          + Nuevo Transporte
        </button>
        <button className="sd01-btn-nueva" style={{ background: '#1d4ed8' }} onClick={() => alert('Carga Excel en desarrollo')}>
          📁 Carga Excel
        </button>
      </div>
      ...
      <div className="ed03-tabla-container">
        <table className="ed03-tabla">
          <thead>
            <tr>
              <th>ID</th><th>Fecha Prog.</th><th>Conductor</th><th>Patente</th>
              <th>Locales</th><th>Estado</th><th>Creado</th><th style={{ width: '180px' }}>Acciones</th>
            </tr>
          </thead>
          <tbody>
        ...
        <div className="ed01-modal-overlay" onClick={() => !guardando && setShowCrearModal(false)}>
          <div className="ed01-modal" style={{ maxWidth: '800px', maxHeight: '90vh' }} onClick={e => e.stopPropagation()}>
            ...
            <div className="ed01-modal-body">
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '16px', marginBottom: '16px' }}>
                <div style={{ position: 'relative' }}>
                  ...
                  <input type="text" value={conductor} onChange={e => handleConductorChange(e.target.value)}
                  ...
                  {showConductorSuggestions && (
                    <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, background: 'white', border: '1px solid #e2e8f0', borderRadius: '6px', boxShadow: '0 4px 12px rgba(0,0,0,0.1)', zIndex: 10, maxHeight: '150px', overflowY: 'auto' }}>
                      {conductorSuggestions.map((c: any) => (
                        <div key={c.id} onClick={() => selectConductor(c)} style={{ padding: '8px 12px', fontSize: '13px', cursor: 'pointer', borderBottom: '1px solid #f1f5f9' }}>{c.nombre_completo}</div>
                      ))}
                    </div>
                  )}
                </div>
                <div style={{ position: 'relative' }}>
                  ...
                  <input type="text" value={patentePrincipal} onChange={e => handlePatentePChange(e.target.value)}
                  ...
                  {showPatentePSuggestions && (
                    <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, background: 'white', border: '1px solid #e2e8f0', borderRadius: '6px', boxShadow: '0 4px 12px rgba(0,0,0,0.1)', zIndex: 10, maxHeight: '150px', overflowY: 'auto' }}>
                      {patentePSuggestions.map((p: any) => (
                        <div key={p.id} onClick={() => selectPatenteP(p)} style={{ padding: '8px 12px', fontSize: '13px', cursor: 'pointer', borderBottom: '1px solid #f1f5f9' }}>{p.numero_patente}</div>
                      ))}
                    </div>
                  )}
                </div>
                  ...
                  {showPatenteASuggestions && (
                    <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, background: 'white', border: '1px solid #e2e8f0', borderRadius: '6px', boxShadow: '0 4px 12px rgba(0,0,0,0.1)', zIndex: 10, maxHeight: '150px', overflowY: 'auto' }}>
                      {patenteASuggestions.map((p: any) => (
                        <div key={p.id} onClick={() => selectPatenteA(p)} style={{ padding: '8px 12px', fontSize: '13px', cursor: 'pointer', borderBottom: '1px solid #f1f5f9' }}>{p.numero_patente}</div>
                      ))}
                    </div>
                  )}
                </div>
                    ...
                    <tbody>
                      {locales.map((loc) => (
                        <tr key={loc.id}>
                          ...
                          <td style={{ padding: '8px' }}>
                            <input type="text" value={loc.nombreLocal} readOnly placeholder="Auto"
              ...
              <div style={{ marginBottom: '16px' }}>
                ...
                <textarea value={observaciones} onChange={e => setObservaciones(e.target.value)} rows={2} placeholder="Observaciones..."
                  style={{ width: '100%', padding: '10px', border: '1px solid #e2e8f0', borderRadius: '6px', fontSize: '13px', fontFamily: 'inherit', resize: 'vertical' }} />
              </div>

              {mensaje && (
                <div style={{ marginBottom: '12px', padding: '10px 14px', borderRadius: '8px', fontSize: '13px', background: tipoMensaje === 'success' ? '#dcfce7' : '#fef2f2', color: tipoMensaje === 'success' ? '#15803d' : '#dc2626' }}>
                  {mensaje}
                </div>
              )}
            </div>
            <div className="ed01-modal-footer">
              ...
              <button className="ed01-btn-save" style={{ background: '#f59e0b' }} onClick={() => handleGuardar('Borrador')} disabled={guardando}>
                {guardando ? '...' : 'Guardar Borrador'}
              </button>
              <button className="ed01-btn-save" onClick={() => handleGuardar('Pendiente')} disabled={guardando}>
