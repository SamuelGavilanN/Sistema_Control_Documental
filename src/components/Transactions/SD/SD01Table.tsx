...
const SD01Table: React.FC<Props> = ({
  rows,
  setRows,
  cantidadFilasAgregar,
  setCantidadFilasAgregar,
  selectedRows,
  setSelectedRows,
  editable = true,
  idDocumento,
  ...
  rows.forEach((row) => {
    if (row.codigoLocal && row.carga) {
      cargaInicialMap[row.codigoLocal] = row.carga;
    }
  });
  ...
  const addMultipleRows = () => {
    ...
    if (targetCount <= currentCount) {
      setRows(rows.slice(0, targetCount));
      return;
    }
  ...
  const updateRow = (id: number, field: keyof SD01Row, value: any) => {
    ...
    if (field === "codigoLocal") {
      const local = locales.find(
        (l) => l.codigo_local.toUpperCase() === value.toUpperCase()
      );
      if (local) {
        setRows(
          updatedRows.map((row) =>
            row.id === id ? { ...row, nombreLocal: local.nombre_local } : row
          )
        );
        return;
      }
  ...
  const fillDown = (field: keyof SD01Row) => {
    ...
    const updatedRows = rows.map((row, index) => {
      ...
      return { ...row, [field]: firstRowValue };
    });
    setRows(updatedRows);
  };
  ...
  const handleDCClick = (row: SD01Row) => {
    setCurrentRow(row);
    setModalOpen(true);
  };

  const handleDCSave = (documentos: any[], localCodigo: string) => {
    const totalCarga = documentos.reduce(
      (sum: number, doc: any) => sum + (doc.cantidadBultos || 0),
      0
    );

    setRows((prevRows) =>
      prevRows.map((row) =>
        row.codigoLocal === localCodigo
          ? { ...row, totalCarga, carga: documentos }
          : row
      )
    );
  };
    ...
    if (e.key === "Tab" && !e.shiftKey) {
      ...
      if (nextRowIndex < rows.length) {
        ...
        setTimeout(() => {
          switch (field) {
            case "codigoLocal":
              codigoLocalRefs.current.get(nextRow.id)?.focus();
              break;
            case "fecha":
              fechaRefs.current.get(nextRow.id)?.focus();
              break;
            case "hora":
              horaRefs.current.get(nextRow.id)?.focus();
              break;
            case "sello":
              selloRefs.current.get(nextRow.id)?.focus();
              break;
            case "pallet":
              palletRefs.current.get(nextRow.id)?.focus();
              break;
          }
        }, 10);
      }
  ...
  const localesRegistrados = rows
    .filter((row) => row.codigoLocal)
    .map((row) => ({
      codigo: row.codigoLocal,
      nombre: row.nombreLocal,
    }));
            ...
            <thead>
              <tr>
                ...
                <th style={{ width: "130px" }}>
                  <div className="th-with-btn">
                    <span>Fecha Entrega</span>
                    <button
                      className="fill-down-btn"
                      onClick={() => fillDown("fechaEntrega")}
                ...
                <th style={{ width: "130px" }}>
                  <div className="th-with-btn">
                    <span>Sello Trasero</span>
                    <button
                      className="fill-down-btn"
                      onClick={() => fillDown("selloTrasero")}
                      title="Copiar primera fila a todas"
                      disabled={!editable}
                    >
                      <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
            ...
            <tbody>
              {rows.map((row, index) => (
                <tr key={row.id}>
                  ...
                  <td>
                    <div className="total-carga-cell">
                      <input
                        type="text"
                        className="table-input total-carga-input"
                        value={row.totalCarga || ""}
                        readOnly
