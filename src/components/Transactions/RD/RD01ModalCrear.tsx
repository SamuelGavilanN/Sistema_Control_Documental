        ...
        <div className="ed01-modal-body">
          ...
          <div className="ed01-row">
            <div className="ed01-field">
              ...
              <input ref={solicitudInputRef} type="text" value={form.numero_solicitud} onChange={e => onSolicitudChange(e.target.value)} placeholder="Ingrese número de solicitud" disabled={camposBloqueados} style={camposBloqueados ? styleDisabled : undefined} />
              {!camposBloqueados && !modoEdicion && <small style={{ color: '#94a3b8', fontSize: '11px' }}>Al escribir se verificará si la solicitud ya existe</small>}
            </div>
            <div className="ed01-field">
              <label>N° Guía *</label>
              <input type="text" value={form.numero_guia} onChange={e => onFormChange('numero_guia', e.target.value)} placeholder="N° Guía" disabled={camposBloqueados} style={camposBloqueados ? styleDisabled : undefined} />
            </div>
          </div>
          ...
          <div className="ed01-row">
            <div className="ed01-field">
              ...
              <input type="text" value={form.codigo_local} onChange={e => onCodigoLocalChange(e.target.value)} placeholder="D001" maxLength={4} disabled={camposBloqueados} style={camposBloqueados ? styleDisabled : undefined} />
            </div>
            <div className="ed01-field">
              <label>Local</label>
          ...
          <div className="ed01-row">
            <div className="ed01-field">
              ...
              <input ref={cantidadInputRef} type="number" value={form.cantidad_bultos || ''} onChange={e => onFormChange('cantidad_bultos', parseInt(e.target.value) || 0)} min="0" placeholder={!modoEdicion && ordenPendiente ? 'Máx: ' + (form.total_bultos - bultosRegistrados) : '0'} style={{ border: '1px solid ' + ((!modoEdicion && ordenPendiente) ? '#f59e0b' : '#e2e8f0'), background: (!modoEdicion && ordenPendiente) ? '#fffdf5' : 'white' }} autoFocus />
              {!modoEdicion && ordenPendiente && <small style={{ color: '#b45309', fontSize: '11px' }}>Máximo permitido: {form.total_bultos - bultosRegistrados} bultos</small>}
            </div>
            <div className="ed01-field">
              <label>Cantidad Total *</label>
              <input type="number" value={form.total_bultos || ''} onChange={e => onFormChange('total_bultos', parseInt(e.target.value) || 0)} min="0" placeholder="0" disabled={camposBloqueados} style={camposBloqueados ? styleDisabled : undefined} />
            </div>
          </div>

          <div className="ed01-field">
            <label>Color *</label>
            <select ref={colorSelectRef} value={form.color} onChange={e => onFormChange('color', e.target.value)} disabled={camposBloqueados} style={camposBloqueados ? styleDisabled : undefined}>
              <option value="">Seleccionar...</option>
              {coloresTipos.map((c: any) => (<option key={c.id} value={c.color}>{c.color}</option>))}
            </select>
          </div>

          {mensaje && (
            <div style={{ marginTop: '12px', padding: '12px 16px', borderRadius: '8px', fontSize: '13px', fontWeight: 500, ...getMensajeStyle() }}>{mensaje}</div>
          )}
        </div>
