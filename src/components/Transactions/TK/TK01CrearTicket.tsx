...
const TK01CrearTicket: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (ticketAbrir && misTickets.length > 0) { const t = misTickets.find(x => x.numero_ticket === ticketAbrir); if (t) { setTicketSeleccionado(t); setRespuestas([]); setShowChatModal(true); localStorage.removeItem('ticket_abrir'); } }
  }, [misTickets]);
  ...
  const handleVerTicket = (t: Ticket) => { setTicketSeleccionado(t); setRespuestas([]); setShowChatModal(true); cargarRespuestas(t.id); };
