...
const moduleTitles: Record<string, string> = {
  ...
  'ai': 'AI01 · Gestión Auditoría Inv',
  'ai-captura': 'AI02 · Captura Auditoria Inv',
  ...
  'sd-documentacion': 'SD02 · Documentación Transporte',
  ...
  return (
    <div className="top-header">
      ...
      <button onClick={toggleDarkMode} className="theme-toggle-btn" title={darkMode ? 'Modo Claro' : 'Modo Oscuro'}
        style={{ width: '34px', height: '34px', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--btn-bg)', border: '1px solid var(--btn-border)', borderRadius: '8px', cursor: 'pointer', transition: 'all 0.15s', marginRight: '10px', color: 'var(--text-muted)' }}>
        {darkMode ? (
          <svg width="16" height="16" viewBox="0 0 16 16" fill="none"><circle cx="8" cy="8" r="3" stroke="currentColor" strokeWidth="1.5"/><path d="M8 1V2M8 14V15M1 8H2M14 8H15M3.05 3.05L3.76 3.76M12.24 12.24L12.95 12.95M3.05 12.95L3.76 12.24M12.24 3.76L12.95 3.05" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"/></svg>
        ) : (
          <svg width="16" height="16" viewBox="0 0 16 16" fill="none"><path d="M13.5 8.5C12.5 11 10 13 7.5 13C5.5 13 3.5 12 2.5 10C4 11 7 10.5 8.5 8C10 5.5 9.5 3 8.5 2C12 2.5 14 5.5 13.5 8.5Z" stroke="currentColor" strokeWidth="1.5" strokeLinejoin="round"/></svg>
        )}
      </button>
      ...
      <div className="notif-area" style={{ position: 'relative', marginRight: '10px' }}>
        ...
        {showNotifMenu && (
          <div className="user-menu" style={{ minWidth: '300px', right: 0, left: 'auto' }} onClick={e => e.stopPropagation()}>
            <div style={{ display: 'flex', justifyContent: 'space-between', padding: '10px 14px', borderBottom: '1px solid var(--border)' }}><span style={{ fontWeight: 600, fontSize: '13px', color: 'var(--text-primary)' }}>Notificaciones</span><button onClick={marcarTodasVisto} style={{ fontSize: '11px', color: '#3b82f6', background: 'none', border: 'none', cursor: 'pointer' }}>Marcar todas vistas</button></div>
